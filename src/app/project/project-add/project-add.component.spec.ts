...
it('should return ProjectView details', () =>
  {
    component.projectsToView = []; 
    spyOn(service,'GetAllProjects').and.returnValues(Observable.of(PROJECTS));
    component.ngOnInit();
    expect(2).toBe(component.projectsToView.length);
    expect("Project 1").toBe(component.projectsToView[0].projectName);
    expect(4).toBe(component.projectsToView[0].numberOfTasks);
    expect(2).toBe(component.projectsToView[0].completedTasks);
  });  
...
it('should have been called onGetAllProject', () =>
{
  component.projectsToView = []; 
  spyOn(service,'GetAllProjects').and.returnValues(Observable.of(PROJECTS));
  component.onGetAllProject();
  expect(2).toBe(component.projectsToView.length);
  expect("Project 1").toBe(component.projectsToView[0].projectName);
  expect(4).toBe(component.projectsToView[0].numberOfTasks);
  expect(2).toBe(component.projectsToView[0].completedTasks);
