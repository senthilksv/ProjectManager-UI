...
describe('TaskmgrEditComponent', () => {
  ...
  beforeEach(async(() => {
    TestBed.configureTestingModule({
      ...
      declarations: [ TaskmgrEditComponent,ProjectNameSearchPipe,UsersSearchPipe ] , 
      providers: [
  ...
  it('Should get correct id from query string and set task details correct', () => {
    var taskdetail: any = { "id": 101, "name": "Task 1", "startDate": Date.now, 
    "endDate" :Date.now, "priority":10, 
        "activeStatus":true, "parentId":102,  "userDetail" : USER,"projectDetail" : PROJECT }

    var parentTaskdetail: any = { "id": 102, "name": "Task 2", "startDate": Date.now, 
        "endDate" :Date.now, "priority":10, 
            "activeStatus":true, "parentId":103,  "userDetail" : USER,"projectDetail" : PROJECT }

   spyOn(taskService,'GetTask').and.returnValue(Observable.of(taskdetail));
  
    component.ngOnInit();

    expect(component.updateTaskId).toBe(101);      
    expect(component.taskDetail.parentId).toBe(102);     
    expect(component.projectName).toBe("Project 1");  
    expect(component.managerName).toBe("User 1 ");  
    expect(component.isParentTaskSelected).toBe(true);  
  });
