...
describe('TaskmgrViewComponent', () => {
  ...
  "endDate" :Date.now, "priority":10, 
    ...
    const PROJECTS : any[] = [
      ...
      "startDate": "2018-07-23","endDate" :"2018-07-28", "priority":20, "userId" : 1, "activeStatus" : false
        }
    ];
  ...
  it('onSelectProject Should return associated task details', () =>
  {
    spyOn(taskService,'GetAllTasks').and.returnValues(Observable.of(TASK_DETAILS));

    component.onSelectProject(PROJECT);    

    expect(component.projectName).toBe("Project 1");   
    expect(component.taskDetailsFiltered.length).toBe(1);   
  ...
  {   
    component.taskDetailsFiltered = TASK_DETAILS;   
    
    spyOn(taskService,'PutTask').and.returnValue(Observable.of("SUCCESS"));
    spyOn(component,'openModal').and.stub();
    component.endTask(1);   
    expect("Task 1 has been closed successfully").toBe(component.results);
    expect(taskService.PutTask).toHaveBeenCalledWith(TASK_DETAILS[0], 1);    
    expect(component.openModal).toHaveBeenCalled();
    expect(component.taskDetail.activeStatus).toBe(false);
  });
